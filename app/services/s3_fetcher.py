...
if __name__ == "__main__":
    s3_fetcher = BTCDataAWSFetcher()
    print(s3_fetcher.get_latest_s3_data())
    ...
    #      # 過濾文件名稱，只保留日期 <= 昨天的檔案
    #     filtered_files = [
    #         obj["Key"] for obj in response["Contents"] if obj["Key"].startswith("2") and obj["Key"].split("_")[0] <= yesterday # 確保是 YYYYMMDD 格式 # 過濾條件
    #     ]
    ...
    # def process_s3_data(self):
    #     """主函數：抓取 S3 數據、計算每日平均值"""
    #     file_names = self._list_s3_files()

    #     if not file_names:
    #         print("✅ S3 沒有新數據")
    #         return None

    #     df = self._fetch_s3_data(file_names)
    #     if df.empty:
    #         print("❌ 無有效數據")
    #         return None
    ...
    #     # 刪除 S3 上已處理的檔案
    #     self._delete_s3_files(file_names)
